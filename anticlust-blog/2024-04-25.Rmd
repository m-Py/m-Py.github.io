...
I implemented a function that generates a data set that includes a numeric response variable that is given by the the combination of an experimental effect and the effect of the item set and some random (normally distributed) error. It also has arguments to adjust the imbalance of the counterbalancing variable (i.e., how many people see condition A with set S1 and how many people see condition A with set S2), and to request that stimuli are assigned to conditions via k-means anticlustering. 
...
The function returns a data frame in long format that has $2N$ rows where $N$ is the number of participants. For purposes of illustration, let's use it to simulate data for four respondents, assuming an effect size of .5 for the experimental condition and an effect size of .3 for the materials (i.e., the expected difference between the stimulus sets): 
...
Next, I define a function to generate a data set and then compute a *t*-test on the response variable by condition. It returns the *p*-value of the *t*-test, which can be used to estimate statistical power. 
...
# significance test for comparing the proportions of significant results between ANOVA and anticlustering
# prop.test(rbind(table(pvalues6 > .05), table(pvalues7 > .05)))
```

We repeat the pattern that the power of the *t*-test is lowest. ANOVA improves power, while statistical power is best when using anticlustering (the effect is even statistically significant and is reproduced when repaating the simulation).
...
Again, we see that the *t*-test suffers a lot when stimulus sets are highly different. Interestingly, the ANOVA approach can deal with the bias pretty well and has the same performance as using anticlustering (which effectively removes the bias).

We can conclude that anticlustering can redemiate the problem of reduced power caused by (random or systematic) differences between item sets, when fixed sets are used and when there is no statistical control of the stimuli. Using ANOVA to employ a statistical control should be done when using stimulus sets instead of the simple *t*-test, preferably combined with anticlustering to remove bias 

Descriptively, combining anticlustering with the simple *t*-test even yielded higher power than the ANOVA approach. I would urge caution in interpreting these results for two reasons: First, only a very small universe of conditions was considered. Second, even better statistical control might be possible with linear mixed models which were specifically discussed to control for variation in stimuli (e.g., Baayen et al., 2008), so a serious comparison should involve this method. However, even the ANOVA approach does not seem to be used in all cases, so anticlustering is certainly useful, especially when considering that potential secondary effects of differences in item sets cannot be controled for via statistical analysis (Lintz et al. 2021). 
