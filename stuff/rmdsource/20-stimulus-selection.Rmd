...
stim <- lexops %>%
  dplyr::filter(PK.Brysbaert >= 0.9) %>%
  mutate(
    CNC = case_when(
      between(CNC.Brysbaert, 1, 2) ~ "CNC-low",
      between(CNC.Brysbaert, 4, 5) ~ "CNC-high",
      TRUE ~ NA_character_ # default case: exclude those
    ),
    BG = case_when(
      between(BG.SUBTLEX_UK, 0, 0.003) ~ "BG-low",
      between(BG.SUBTLEX_UK, 0.009, 0.013) ~ "BG-high",
      TRUE ~ NA_character_ # default case: exclude those
    )
  ) %>% 
  filter(!is.na(CNC), !is.na(BG), !is.na(Length), !is.na(Zipf.SUBTLEX_UK)) %>% 
  mutate(condition = paste(CNC, BG, sep = " | "))

stim$matches <- matching(stim[, c("Length", "Zipf.SUBTLEX_UK")], groups = stim$condition)
full_matches <- which(table(stim$matches) == 4)
subchoice <- stim[stim$matches %in% full_matches, ]

objectives <- sapply(
  unique(subchoice$matches), 
  function(x) sum(dist(subchoice[subchoice$matches == x, c("Length", "Zipf.SUBTLEX_UK")]))
)

obj <- data.frame(
  match = unique(subchoice$matches),
  objectives = objectives
)

obj <- obj %>% arrange(objectives)
best <- obj$match[1:25]

selected <- stim %>% 
  filter(matches %in% !!best) %>% 
  select(CNC.Brysbaert, BG.SUBTLEX_UK, Length, Zipf.SUBTLEX_UK, matches, condition) %>% 
  arrange(matches, condition)

mean_sd_tab(
  selected[, c("CNC.Brysbaert", "BG.SUBTLEX_UK", "Length", "Zipf.SUBTLEX_UK")], 
  selected$condition
)
...
full_matches <- which(table(oasis$matches) == 3)
subchoice <- oasis[oasis$matches %in% full_matches, ]

objectives <- sapply(
  unique(subchoice$matches), 
  function(x) sum(dist(subchoice[subchoice$matches == x, covariates]))
)
...
selected <- subchoice %>% 
  filter(matches %in% !!best) %>% 
  select(Valence_mean, Arousal_mean, beauty_mean, matches) %>% 
  arrange(matches)

# do anticlustering for optimization
selected$ac <- anticlustering(
  selected[, 1:3],
  K = 3,
  categories = selected$matches
)
...
The `anticlust` package did not discard any items---all 96 items are 
part of the output. The difference with regard to frequency is smaller, 
but the covariates are more similar. In general, `anticlust` will focus 
on similarity in covariates more than dissimilarity with regard to the 
