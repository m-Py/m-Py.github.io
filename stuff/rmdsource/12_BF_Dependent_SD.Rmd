...
the Levene test should usually be preferred. The Levene test is 
well-known because it is commonly used to test the assumption of 
"homogeneity" in variances before conducting an ANOVA. However, the 
logic of significance tests forbids that the output of the test can be 
interpreted as evidence of variance equality; we may only conclude the 
absence of variance homogeneity if the *p* value is small. Wouldn't it 
be nice to actually have a test of the equality of variances whose 
result may actually give evidence in favor of variance homogeneity. This 
is what a Bayes factor can do---and I actually think this functionality 
is needed---for which reason I implemented it in `R`. You can install my 
small package `varBF` as follows: 
...
it. The code (it is really not that much) for the two functions 
`depvarBF()` and `indepvarBF()` is available from 
[here](https://github.com/m-Py/varBF/blob/master/R/depvarBF.R) and 
[here](https://github.com/m-Py/varBF/blob/master/R/indepvarBF.R).
I am rather optimistic that some of the points discussed here 
