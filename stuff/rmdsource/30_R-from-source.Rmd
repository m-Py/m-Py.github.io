...
If we wish to reproduce specific settings from a CRAN machine for reproducing errors, we can set compilation flags in the file config.site. For example for the CRAN machine noLD (no long doubles)
...
```
CONFIG_SITE="$PWD/config.site" ./configure --enable-lto=R --disable-long-double
```

Or, if Java makes problems:

```
CONFIG_SITE="$PWD/config.site" ./configure --enable-lto=R --disable-long-double --disable-java
```
...
```R
dependencies <- unlist(tools::package_dependencies("anticlust"))
dependencies <- c(dependencies, unlist(tools::package_dependencies("anticlust",  which = "Suggests")))
dependencies <- setdiff(dependencies, "gurobi") # gurobi not on CRAN
...
When a suggested package is not available, use:

```
_R_CHECK_FORCE_SUGGESTS_=false ./R CMD check --as-cran tar-ball.tar.gz
```
