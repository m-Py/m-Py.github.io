...
0. Appendix A1 "Essential programs and libraries" in "R Installation and 
...
$\to$ R Sources
2. Unpack the sources: `tar -xf R-devel.tar.gz`
3. Go to directory (`cd R-devel`) and check out the file INSTALL and 
...
5. `make check` to test if the installation was successful.
6. `make pdf` and `make info` to create manuals
...
8. It is also possible to install `R` devel on the system by using `make 
install` but I guess this is not recommended (I suppose this replaces 
the current stable installation of `R`)
