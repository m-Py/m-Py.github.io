...
group t-test. I show that the number of possible permutations usually 
assumed in the statistical literature is incorrect when the two groups 
have the same size.*
...
it is statistically significant. Unlike the traditional *t*-test, this 
procedure to declare significance seems to be legitimate even if the 
data are non-normal or if other assumptions of the traditional test are 
violated.
...
statistical literature on permutation tests. For example, Ernst (2004) 
...
format(n_partitions(N = 30, K = 3), big.mark = ",")` (~ 900 billion) and 
not over over 5.5 trillion (approx. 5.5 trillion divided by $6$). The 
general formula to compute the number of ways to partition $N$ elements 
into $K$ equal-sized groups is given as follows (see Papenberg & Klau, 
2019): 
...
https://doi.org/10.31234/osf.io/3razc. Manuscript submitted for 
publication. 
